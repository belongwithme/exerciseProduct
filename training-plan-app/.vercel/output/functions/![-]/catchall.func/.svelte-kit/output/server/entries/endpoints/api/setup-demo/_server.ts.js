...
const POST = async ({ locals: { supabase, safeGetSession } }) => {
  ...
  try {
    const { data: existingPlans, error: checkError } = await supabase.from("workout_plans").select("id").eq("user_id", userId).limit(1);
    if (checkError) {
    ...
    const { data: createdPlans, error: planError } = await supabase.from("workout_plans").insert(plans).select("id, name");
    if (planError) {
    ...
    if (stage1Plan) {
      ...
      for (const ex of stage1Exercises) {
        const { data: exercise } = await supabase.from("exercises").select("id").eq("name", ex.exerciseName).single();
        if (exercise) {
          planExercises.push({
    ...
    if (planExercises.length > 0) {
      const { error: exerciseError } = await supabase.from("plan_exercises").insert(planExercises);
      if (exerciseError) {
