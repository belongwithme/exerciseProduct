...
<script lang="ts">
  ...
  async function loadUserStats() {
    try {
      // 并行获取各项统计数据
      const [plansResponse, logsResponse, streakResponse] = await Promise.all([
        fetch('/api/plans'),
        fetch('/api/logs'),
      ...
      if (plansResponse.ok) {
        ...
        stats.plans = plans.length || 0;
      }

      // 处理训练日志数量
      if (logsResponse.ok) {
        const logs = await logsResponse.json();
        stats.workouts = logs.length || 0;
      }

      // 处理连续打卡天数
      if (streakResponse.ok) {
        const result = await streakResponse.json();
        stats.streak = result.streak_days || 0;
      }

    } catch (error) {
      console.error('加载统计数据失败:', error);
      // 使用默认值
      stats = { plans: 0, workouts: 0, streak: 0 };
    }
  }
